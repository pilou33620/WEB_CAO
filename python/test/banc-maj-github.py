...
des mises à jour GitHub au démarrage de web_CAO.py.
"""
import os
import sys
import unittest
...
class TestMajGitHub(unittest.TestCase):
    ...
    def test_01_git_non_installe(self):
        ...
        with patch("shutil.which", return_value=None):
            maj = web_CAO.verifier_et_appliquer_maj(ROOT)
    ...
    def test_07_mise_a_jour_avec_modifications_locales_stash(self):
        ...
        def fake_run(cmd, *args, **kwargs):
            ...
            if "status" in cmd:
                return MagicMock(returncode=0, stdout=" M web_CAO.py\n")  # dirty
    ...
    def test_08_option_sans_maj_cli(self):
        ...
        with patch("web_CAO.verifier_et_appliquer_maj") as mock_verifier:
            with patch("web_CAO.start_server", return_value=0):
                web_CAO.main(["--sans-maj", "--local", "--port", "0", "--sans-navigateur"])
    ...
    def test_09_variable_environnement_evite_boucle_redemarrage(self):
        ...
        with patch.dict(os.environ, {"WEB_CAO_DEJA_MAJ": "1"}):
            with patch("web_CAO.verifier_et_appliquer_maj") as mock_verifier:
                with patch("web_CAO.start_server", return_value=0):
                    web_CAO.main(["--local", "--port", "0", "--sans-navigateur"])
                    mock_verifier.assert_not_called()
