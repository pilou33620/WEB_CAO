...
SRC = os.path.join(ROOT, "editeur-pcb.html")
DST = os.path.join(ROOT, "dist", "editeur-pcb.html")
...
with io.open(DST_JS, "w", encoding="utf-8") as f:
    f.write(js_bundle)

print("écrit : %s et %s  (%d Ko, %d feuille(s) de style, %d scripts)"
      % (os.path.relpath(DST, ROOT), os.path.relpath(DST_JS, ROOT), os.path.getsize(DST) // 1024, n_css, len(scripts)))
