...
function closeZone(){
  ...
  def = prompt("Nom du net à associer à ce plan de cuivre (ex: GND, 5V)\nLaissez vide pour un plan de cuivre isolé.", def);
  if(def===null) { S.zoneDraft=Z; return; } // annuler la création
  const z={id:S.nextId++,l:Z.l,net:def.trim(),pts:Z.pts.map(p=>({x:r3(p.x),y:r3(p.y)}))};
  S.zones.push(z);
  clearSel();S.sel.zones.add(z);
  touch();refreshPanels();draw();
  hint("Zone créée sur "+cuId(z.l,S.cu)+(def?" et rattachée au net "+def:"")+
       " — le net peut être modifié dans le panneau Propriétés.");
}
...
function fullBoardZone(){
  ...
  const nets=netTable();
  let def=S.hlNet||(nets.find(n=>isPower(n.name))||{}).name||"";
  def = prompt("Nom du net à associer à ce plan pleine carte (ex: GND, 5V)\nLaissez vide pour un plan isolé.", def);
  if(def===null) return;
  const z={id:S.nextId++,l:S.active,net:def.trim(),pts:boardZonePts()};
  S.zones.push(z);
  clearSel();S.sel.zones.add(z);
  touch();refreshPanels();draw();
  hint("Plan pleine carte posé sur "+cuId(S.active,S.cu)+" : le net peut être modifié dans les Propriétés.");
}
...
const WS_KEY="pcb.espace-travail.v1";
