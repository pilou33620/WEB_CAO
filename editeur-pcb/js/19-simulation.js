...
const SIM_PCB={
  ...
  pdnRails:function(){
    ...
    const aDesNets = (typeof S!=="undefined"&&S&&((S.nets&&Object.keys(S.nets).length>0)||(Array.isArray(S.tracks)&&S.tracks.length>0)));

    if(aDesNets){
      if(S.selectedNet&&!isGnd(S.selectedNet)) rails.add(S.selectedNet);
      if(S.curNet&&!isGnd(S.curNet)) rails.add(S.curNet);

      if(Array.isArray(S.fps)){
        for(const fp of S.fps){
          const isCapa=fp.type==="capacitor"||/^[cC]/i.test(fp.ref)||/[pnum]F/i.test(fp.value);
          if(!isCapa) continue;
          const pads=padsWorld(fp);
          const nets=pads.map(p=>p.net||(fp.nets&&fp.nets[p.n])).filter(Boolean);
          const hasGnd=nets.some(isGnd);
          if(hasGnd){
            for(const n of nets){
              if(!isGnd(n)) rails.add(n);
            }
          }
        }
      }
      if(Array.isArray(S.tracks)){
        for(const t of S.tracks){
          if(t&&t.net&&isPwr(t.net)) rails.add(t.net);
        }
      }
      if(S.nets){
        for(const nom of Object.keys(S.nets)){
          if(nom&&isPwr(nom)) rails.add(nom);
        }
      }
    }
    ...
    if(aDesNets&&S.nets){
      const tous=Object.keys(S.nets).filter(n=>n&&!isGnd(n));
  ...
  /* Tous les nets de la carte */
  pdnTousNets:function(){
    if(typeof S==="undefined"||!S) return [];
    const isGnd=n=>/^(gnd|0v|vss|ground|earth|mass|masse|[adp]?gnd.*)$/i.test(String(n).trim());
    const set=new Set();
    if(S.nets) for(const n of Object.keys(S.nets)) if(n&&!isGnd(n)) set.add(n);
    if(Array.isArray(S.tracks)) for(const t of S.tracks) if(t&&t.net&&!isGnd(t.net)) set.add(t.net);
